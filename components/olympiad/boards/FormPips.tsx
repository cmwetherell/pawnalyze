...
import type { FormEntry } from '@/lib/olympiad/types';
...
export default function FormPips({ form, lastRound, size = 'sm' }: FormPipsProps) {
  ...
  return (
    <span className="inline-flex items-center gap-[3px]" role="img" aria-label={form.map(f => `R${f.round} ${WORD[f.score]} vs ${displayName(f.oppName)}`).join(', ') || 'No games yet'}>
      {rounds.map(r => {
        const f = byRound.get(r);
        return f ? (
          <span
            key={r}
            title={`R${r}: ${WORD[f.score]} vs ${displayName(f.oppName)} (${f.oppRating || 'unrated'}), ${f.colour === 'w' ? 'white' : 'black'}`}
